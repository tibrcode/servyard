...
export function BookingManagement({
  ...
  const translations = {
    ...
    customer: t.booking.customer || 'Customer',
    service: t.booking.selectService || 'Service',
    date: 'Date',
    time: t.booking.selectTime || 'Time',
    price: t.provider.price || 'Price',
    notes: t.booking.notes || 'Notes',
    noNotes: 'No notes',
    confirm: t.booking.confirm || 'Confirm',
    reject: t.booking.reject || 'Reject',
    cancel: 'Cancel',
    complete: t.booking.completed || 'Complete',
    markNoShow: t.provider.bookingStatusNoShow || 'No Show',
    call: t.booking.call || 'Call',
    updating: t.ui?.loading || 'Updating...',
    confirmed: t.booking.statuses?.confirmed || 'Confirmed',
    rejected: t.booking.statuses?.rejected || 'Rejected',
    cancelled: t.booking.statuses?.cancelled || 'Cancelled',
    completed: t.booking.statuses?.confirmed_completed || 'Completed',
    updateSuccess: 'Booking updated',
    updateFailed: 'Update failed',
    statistics: 'Statistics',
    totalBookings: 'Total Bookings',
    pendingBookings: t.booking.statuses?.pending || 'Pending',
    confirmedBookings: t.booking.statuses?.confirmed || 'Confirmed',
    revenue: t.provider.revenue || 'Revenue',
    statuses: {
      all: t.provider.bookingStatusAll || 'All',
      pending: t.booking.statuses?.pending || 'Pending',
      confirmed: t.booking.statuses?.confirmed || 'Confirmed',
      cancelled: t.booking.statuses?.cancelled || 'Cancelled',
      completed: t.booking.statuses?.confirmed_completed || 'Completed',
      'no-show': t.provider.bookingStatusNoShow || 'No Show',
    },
  };
  ...
  const renderBookingCard = (booking: Booking) => {
    ...
    return (
      <Card key={booking.booking_id} data-booking-id={booking.booking_id} id={`provider-booking-${booking.booking_id}`} className="bg-muted/50 hover:bg-muted/70 transition-colors">
